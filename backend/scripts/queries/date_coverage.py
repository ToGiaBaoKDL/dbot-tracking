#!/usr/bin/env python3
"""Check for missing trading dates (gaps) in the data."""
...
def query(start_date: date, end_date: date):
    engine = _get_engine()
    sql = """
        SELECT record_date, COUNT(DISTINCT symbol) as symbol_count
    ...
    logger.info("%-12s %10s", "Date", "Records")
    logger.info("-" * 25)
    for row in rows:
        logger.info("%-12s %10d", str(row[0]), row[1])


def main() -> int:
    parser = argparse.ArgumentParser(description="Check data coverage by date")
    parser.add_argument(
